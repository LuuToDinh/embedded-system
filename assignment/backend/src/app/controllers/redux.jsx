...
async function getDaydata(feed) {
  const url = `https://io.adafruit.com/api/v2/dinhluu/feeds/${feed}/data?limit=20`;
...
async function getPumpAPI() {
  const url = 'https://io.adafruit.com/api/v2/dinhluu/feeds/pump/data?limit=1';
  const response = await fetch(url);
  const data = await response.json();
