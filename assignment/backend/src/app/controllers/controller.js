...
class AppController {
  ...
  async getDataDevices(req, res, next) {
    const url = `https://io.adafruit.com/api/v2/dinhluu/feeds/${req.body.name}/data?limit=20`;
    ...
    let result = {
      ...
      value: data[0].value,
      label: data.map((i) =>
        i.created_at.substring(
          i.created_at.indexOf('T') + 1,
          i.created_at.indexOf('Z')
        )
      ),
  ...
  async getDataPump(req, res, next) {
    const url =
      'https://io.adafruit.com/api/v2/dinhluu/feeds/pump/data?limit=1';
    const response = await fetch(url);
  ...
  async setDataPump(req, res, next) {
    ...
    await mqttclient.publish('dinhluu/feeds/pump', value.toString());
    return res
      .status(200)
      .json({ message: `Change value pump to ${value} successfully` });
  }

  async getDataOxy(req, res, next) {
    const url =
      'https://io.adafruit.com/api/v2/dinhluu/feeds/oxygenpump/data?limit=1';
    const response = await fetch(url);
    const result = await response.json();
    return res.status(200).json(result);
  }
  ...
  async setDataOxy(req, res, next) {
    ...
    if (value < 0 || value > 4)
      return res.status(400).json({ message: 'Invalid value' });
    await mqttclient.publish('dinhluu/feeds/oxygenPump', value.toString());
    return res
      .status(200)
      .json({ message: `Change value oxygenPump to ${value} successfully` });
  }
}
